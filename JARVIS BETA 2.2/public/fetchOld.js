    ...
    function formatCodeBlocks(message) {
        ...
        const formattedMessage = message.replace(
            ...
            (match, code) => {
                ...
                // Adjust indentation for each line
                const indentedLines = lines.map(line => {
                    // Calculate the number of spaces to add or remove
                    const indent = line.match(/^\s*/)[0].length;
                    const spacesToAdd = indent - minIndent;
    
                    // Add or remove spaces based on the minimum indentation
                    let indentedLine = ' '.repeat(spacesToAdd) + line.trim();
...
async function fetchOldData(username) {
    ...
    try {
        ...
        for (let i = 0; i < jsonResponse.length; i++) {
            let checkcode=isCode(jsonResponse[i].response);
            ...
            if(checkcode)
            {
                ...
                <div class="response-message items-start w-full flex flex-col p-2">
                    ...
                    <div class="message w-full">
                        ${formatCodeBlocks(jsonResponse[i].response)}
                    </div>
                ...
                <p class="font-bold">JARVIS</p>
                    <div class="message">
                        ${formatText(jsonResponse[i].response)}
