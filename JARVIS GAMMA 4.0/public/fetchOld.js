...
async function fetchOldTableNames()
{
    ...
    try{
        ...
        for(let i=0; i<jsonResponse.length; i++) {
            ...
            if(jsonResponse[i]!='authentication')
            {
                const addSummary = `<input id="table${i}" type="button" value="${jsonResponse[i]}" class="summary rounded hover:cursor-pointer h-fit text-center p-2 my-1" />`;
